...
import { getStudentIncentivesDB, getStudentIncentiveByIdDB } from '../../services/postgresClient';

const logger = new Logger();

export const getStudentIncentives = async (request: Request, response: Response) => {
    try {
        const { pageNumber, pageSize, studentId, userId } = request.query;
        const result = await getStudentIncentivesDB(
...
export const getStudentIncentiveById = async (request: Request, response: Response) => {
    try {
        const { id } = request.params;
        const result = await getStudentIncentiveByIdDB(id);
